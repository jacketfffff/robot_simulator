...
void BaseController::joy_velCallback(const geometry_msgs::TwistConstPtr &msg)
{
    ...
    user_cmd_vel.cmd_right_wheel = right_vel*60.0/(M_PI * BASE_MODEL_.Wheel_Diameter/1000.0);
    user_cmd_vel.cmd_left_wheel  = left_vel *60.0/(M_PI * BASE_MODEL_.Wheel_Diameter/1000.0);
...
{
    wheel_vel[5] = -user_cmd_vel.cmd_left_wheel;
    wheel_vel[6] = user_cmd_vel.cmd_right_wheel;
...
void BaseController::joint_lock()
{
    serialManager -> send(left_hip_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(left_hip1_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(left_knee_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(left_ankle_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(right_hip_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(right_hip1_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(right_knee_lock, LOCK_SIZE);
    usleep(50000);
    serialManager -> send(right_ankle_lock, LOCK_SIZE);
    usleep(50000);
    ROS_INFO_STREAM("ROBOT JOINT LOCKED");
}