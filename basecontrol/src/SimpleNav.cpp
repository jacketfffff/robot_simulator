...
int main(int argc, char* argv[])
{
    ...
    //actionlib
    ActionLib actionLib(serial_addr);

    bool nav_on{},nav_pause{},newGoal{true};
    std::vector<targetPose> targetPsoeArray{};
    getPoseArray(targetPsoeArray);
    auto iter = targetPsoeArray.begin();
    while(ros::ok())
    {
        ...
        if (nav_on && !targetPsoeArray.empty() && newGoal)
        {
            if (iter != targetPsoeArray.end())
            {
