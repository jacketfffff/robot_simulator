...
{   
    ...
    {
        control_trigger_ = Jump;
    }
    else if( msg -> buttons[2] ) //X
    {
        control_trigger_ = JointLock;
    }
    else if( msg -> buttons[3] ) //Y
    {
        control_trigger_ = Squat;
    }
    else if( msg -> axes[7] ) // up
    {
        control_trigger_ = MoveForward;
    }
    else if( msg -> axes[7] == -1 ) //down
    {
        control_trigger_ = MoveBack;
    }
    else if( msg -> axes[6] ) //left
    {
        control_trigger_ = TurnLeft;
    }
    else if( msg -> axes[6] == -1) //right
    {
        control_trigger_ = TurnRight;
    }
    //else
    //{
    //    control_trigger_ = Default;
